...
class UpdateAlternative():
    def __init__(self, kwargs=Sequence[str]) -> None:
        parser = argparse.ArgumentParser()
        parser.add_argument('-v', '--version', nargs=1, type=int, help='Clang version')
        parser.add_argument('-p', '--priority', nargs=1, type=int, help='update-alternatives priority')
    ...
    def add_install(self, name: str) -> None:
        self._command.extend(
            ['--install', f'/usr/bin/{name}', str(name), f'/usr/bin/{name}-{self._version}', str(self._priority)]
        )

    def add_slave(self, name: str) -> None:
        self._command.extend(
            ['--slave', f'/usr/bin/{name}', str(name), f'/usr/bin/{name}-{self._version}']
        )
